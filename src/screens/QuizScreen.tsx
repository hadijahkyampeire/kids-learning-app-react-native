...
} from "../components/MultiChoiceQuestion";
...
const QuizScreen: React.FC = () => {
  ...
  const allForLevel: Question[] = level
    ? (questionsData as any)[level] ?? []
    : [];
  ...
  const autoMarkMulti = async (finalSel: string[]) => {
    const corr = Array.isArray(currentQ.correct)
      ? (currentQ.correct as string[])
      : [];
    ...
    setFeedback(
      ok
        ...
        : `Incorrect: ${
            currentQ.feedback?.incorrect ?? "Review the correct set."
          }`
    );
    setAnswers((prev) => [
      ...prev,
  ...
  const markAndStore = async () => {
    ...
    switch (currentQ.type) {
      ...
      case "multiChoice": {
        const correctArr = Array.isArray(currentQ.correct)
          ? (currentQ.correct as string[])
          : [];
      ...
      case "multiText": {
        ...
        (currentQ as any).blanks.forEach((b: any) => {
          ...
          const pats = buildPatterns({
            accepted: b.accepted,
            correct: b.correct,
            correctRegex: b.correctRegex,
          });
    ...
    // play sound
    if (isCorrect) await playCorrect();
    ...
    setFeedback(
      isCorrect
        ...
        : `Incorrect: ${
            ...
            "Check spelling/format and try again."
          }`
    );
  ...
  const renderByType = () => {
    ...
    switch (currentQ.type) {
      case "choice": {
        const corr =
          typeof currentQ.correct === "string" ? currentQ.correct : "";
        return (
          <ChoiceQuestion
  ...
  if (questions.length === 0) {
    return (
      <ImageBackground
        source={{
          uri: "https://kids-app-images.s3.us-east-1.amazonaws.com/quiz-bg.jpg",
        }}
    ...
    >
      ...
      <View style={styles.content}>
        <Text style={styles.h1}>
          {capitalize(subject)} {currentQ.topic ? `• ${currentQ.topic}` : ""}
        </Text>
        <Text style={styles.h3}>
          {capitalize(level)} / {klass?.toUpperCase?.()}
        </Text>

        <View style={styles.card}>
          <Text style={styles.h2}>{currentQ.text}</Text>

          <View style={{ marginTop: 8 }}>{renderByType()}</View>

          <View style={styles.controlsRow}>
            {!locked && (
              <TouchableOpacity
                style={styles.submitButton}
                onPress={markAndStore}
              >
